...
<script>
     var TableRetailer = $('#TableRetailer').dataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": url_gb+"/admin/RetailerWaiting/ListRetailer",
            "data": function ( d ) {
                //d.myKey = "myValue";
     ...
     $('body').on('click','.btn-edit',function(data){
        ...
        $.ajax({
            method : "GET",
            url : url_gb+"/admin/RetailerWaiting/Edit/"+id,
            dataType : 'json'
        }).done(function(rec){
            console.log(rec);
    ...
    $('#FormEdit').validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                method : "POST",
                url : url_gb+"/admin/RetailerWaiting/Update/"+id,
                dataType : 'json',
                data : $(form).serialize()
            }).done(function(rec){
                btn.button("reset");
    ...
    $('body').on('click','.btn-changeStatus',function(data){
        ...
        var id = $(this).data('id');
        swal({
            title: "Approve Retailer’s Email?",
            type: "warning",
            ...
            if(data){
                $.ajax({
                    method : "GET",
                    url : url_gb+"/admin/RetailerWaiting/changeStatus/"+id,
                    dataType : 'json'
                }).done(function(rec){
                    btn.button("reset");
                    if(rec.status==1){
