    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <div class="table-responsive">
                <table class="table table-hover" id="TableAdvertisement">
                    <thead>
                        <tr>
                            <th>No.</th>
                            <th>Title</th>
...
@section('js_bottom')
<script>
     var TableAdvertisement = $('#TableAdvertisement').dataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": url_gb+"/admin/Advertisement/ListAdvertisement",
            "data": function ( d ) {
                //d.myKey = "myValue";
    ...
    $('body').on('click','.btn-edit',function(data){
        ...
        $.ajax({
            method : "GET",
            url : url_gb+"/admin/Advertisement/"+id,
            dataType : 'json'
        }).done(function(rec){
            $('#edit_id').val(rec.id);
            $('#edit_title').val(rec.adv_title);
            $('#edit_detail').val(rec.adv_detail);

            if (rec.adv_image) {
                var max = 0;
                var img = rec.adv_image;
            }else{
                var max =1;
                var img = null;
            }
    ...
    $('#FormAdd').validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                method : "POST",
                url : url_gb+"/admin/Advertisement",
                dataType : 'json',
                data : $(form).serialize()
            }).done(function(rec){
                btn.button("reset");
                if(rec.status==1){
                    TableAdvertisement.api().ajax.reload();
    ...
    $('#FormEdit').validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                method : "POST",
                url : url_gb+"/admin/Advertisement/CheckEdit/"+id,
                dataType : 'json',
                data : $(form).serialize()
            }).done(function(rec){
                btn.button("reset");
...
$("body").on("click",".btn-delete",function(){
        ...
        swal({
            ...
            if(data){
                $.ajax({
                    method : "POST",
                    url : url_gb+"/admin/Advertisement/Delete/"+id,
                    data : {ID : id}
                }).done(function(data){
                    if(data){
...
$('#photo').orakuploader({
    ...
    orakuploader_main_path : 'uploads/advertisements/',
    orakuploader_thumbnail_path : 'uploads/advertisements/',                
    orakuploader_thumbnail_real_path : asset_gb+'uploads/advertisements/',  
