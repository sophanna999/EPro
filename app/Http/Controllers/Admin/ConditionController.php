...
class ConditionController extends Controller
{
    public function index()
    {
		...
		$data['sub_menu']   = 'CustomerCondition';
		$data['title_page'] = 'Customer Condition Form';
		$data['menus']      = \App\Models\AdminMenu::ActiveMenu()->get();
		$data['permission'] = \AdminPermission::CheckPermission($data['main_menu']);
        return view('Admin.customer_condition',$data);
    }
    ...
    public function ListCustCondition()
    {
        $result = \App\Models\Condition::where('type', 'C')->select();
        return \Datatables::of($result)
        ->addIndexColumn()
        ->addColumn('action',function($rec){
            $permission = \AdminPermission::CheckPermission('CustomerCondition');
            if ($permission['U'] == "T") {
